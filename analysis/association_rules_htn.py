"""
Association Rule Mining — 고혈압 이상군 전용 (HTN)

대상 파일: data/preprocessed/health_data_2024_preprocessed_HTN.csv
  - 고혈압 관련 속성만 포함
  - 정상군(고혈압=0) 제외 → 주의(1) / 위험(2) 만 포함
...
실행:
    python analysis/association_rules_htn.py
"""
...
RESULT_DIR = f"result/analysis/HTN_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
os.makedirs(RESULT_DIR, exist_ok=True)
print(f"결과 저장 경로: {RESULT_DIR}\n")

DATA_PATH    = "data/preprocessed/health_data_2024_preprocessed_HTN.csv"
TARGET       = '고혈압'
TARGET_COLOR = '#3498DB'
...
print(f"  고혈압 단계 분포: {df[TARGET].value_counts().sort_index().to_dict()}  (1=주의, 2=위험)")
...
print(f"  고혈압 단계 분포: {df_disc[TARGET].value_counts().sort_index().to_dict()}")
...
# 고혈압 관련 규칙 (결론부에 고혈압 포함)
rules_htn = rules[
    rules['결론부(then)'].str.contains(TARGET)
].reset_index(drop=True)

print(f"\n  [고혈압 관련 규칙 수: {len(rules_htn)}개]")
if len(rules_htn):
    print(rules_htn[display_cols].head(TOP_N).round(4).to_string(index=False))
    rules_htn[display_cols].to_csv(f"{RESULT_DIR}/association_rules_htn.csv",
                                   index=False, encoding="utf-8-sig")

# 바차트: 고혈압 관련 규칙 상위 TOP_N
if len(rules_htn):
    top_n  = rules_htn.head(TOP_N)
    labels = (top_n['조건부(if)'] + '  →  ' + top_n['결론부(then)'])
    colors = ['#3498DB' if '위험' in c else '#85C1E9'
    ...
    ax.set_title(f"고혈압 관련 연관 규칙 상위 {TOP_N}개 (Lift 기준)\n"
                 f"진한색=위험, 연한색=주의", fontsize=12)
    plt.tight_layout()
    plt.savefig(f"{RESULT_DIR}/association_rules_htn_bar.png", dpi=150, bbox_inches='tight')
    plt.show()
    print(f"\n  저장: {RESULT_DIR}/association_rules_htn_bar.png")
